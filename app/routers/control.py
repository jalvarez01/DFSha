...
@router.post("/{path:path}/blocks/{index}/allocate", response_model=AllocatedBlock)
def allocate_block(
    path: str,
    index: int,
    payload: BlockAllocateRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> AllocatedBlock:
    """Escritura CRUD de un único bloque (RF3 — decisión de equipo: acceso
    CRUD, no WORM): actualiza un bloque ya existente o anexa uno nuevo al
    final del archivo, SIN tocar los demás bloques ni descartar el resto
    del plan (a diferencia de `allocate`, que reemplaza todo el archivo).

    - `index < len(blocks)`: reemplaza ese bloque. Se descarta la fila
      vieja (su contenido en el DataNode anterior queda huérfano, igual que
      en el re-allocate completo de `allocate`) y se crea una nueva con
      `block_id` nuevo, posiblemente en otro DataNode.
    - `index == len(blocks)`: anexa un bloque nuevo al final (crecer el
      archivo). No se permite dejar huecos: `index > len(blocks)` es
      `400 invalid_block_index`.

    IMPORTANTE sobre el enrutado: esta ruta se registra ANTES que
    `/{path:path}/allocate` a propósito. Como `{path:path}` es un
    catch-all greedy, una URL como `/files/foo/blocks/0/allocate` también
    calzaría con el patrón `{path:path}/allocate` del allocate de archivo
    completo (con path="foo/blocks/0"); FastAPI/Starlette prueban las
    rutas en el orden en que se registraron y usan la primera que calce,
    así que esta debe ir primero o quedaría inalcanzable.

    El cliente sube el contenido con `PUT {datanode}/blocks/{block_id}` y
    cierra con el `/confirm` de siempre — `confirm` ya soporta confirmar un
    subconjunto de bloques sin afectar a los que no cambiaron.
    """
    ...
    current = list(file_row.blocks)  # ya vienen ordenados por block_index
    if index < 0 or index > len(current):
        raise HTTPException(
            ...
            detail=ErrorResponse(
                ...
                message=(
                    f"Índice de bloque {index} inválido para {path} "
                    f"(el archivo tiene {len(current)} bloque(s); solo se puede "
                    f"reemplazar 0..{len(current) - 1} o anexar en {len(current)})"
                ),
            ).model_dump(),
        )
